 ...
 */
function _build_elastic_search_query(
    $field,
    $keyword,
...
function _get_field_keyword_pairs($form_input)
{
    ...
    */
    ...
    return array(
        'table' => $table,
        'field_keyword_pairs' => $field_keyword_pairs,
        'search_from' => 0,
    );
}
...
function _run_elastic_main_search($table, $field_keyword_pairs, $size = 100)
{
    ...
    try {
        ...
        return array(
            'search_hits_count' => $search_hits_count,
            'main_search_hits' => $main_search_hits,
        );
    } catch (\Exception $e) {
 ...
 */
function get_main_search_hits_table(
    $main_search_hits,
    $main_search_hits_count
)
{
    $output = '';
    if (!empty($main_search_hits)) {
        ...
        $output .= theme(
            'table',
            array('header' => array(), 'rows' => $chunks[$current_page])
        );
...
function get_search_hits_table($search_hits, $table)
{
    ...
    if (isset($_GET['sort']) and isset($_GET['order'])) {
        $sorted_hits_records = sort_2d_array_by_value(
            $search_hits,
            $_GET['order'],
            $_GET['sort']
        );
    ...
    foreach ($sorted_hits_records as $values) {
        ...
        $records = db_query(
            'SELECT DISTINCT(table_field), page_link FROM tripal_elasticsearch_add_links WHERE table_name=:table_name',
    ...
    $output = theme(
        ...
        array(
            ...
            'attributes' => array('id' => 'elasticsearch_hits_table'),
        )
    );
    $output .= theme('pager', array('quantity', count($rows)));
...
function tripal_elasticsearch_add_block($indexed_table, $fields)
{
    ...
    $delete_table_name = db_delete('tripal_elasticsearch')->condition(
        'table_name',
        $indexed_table
    )->execute();
    $i = 0;
...
{
    $delete_table_name = db_delete('tripal_elasticsearch_add_links')->condition(
        'table_name',
        $table_name
    )->execute();
    $columns = db_query(
        'SELECT table_field FROM tripal_elasticsearch WHERE table_name=:table_name',
